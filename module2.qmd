...
# What happens during synthesis research? NEW
...
# Data preparation (cleaning and harmonizing) NEW
...
Metadata for ecological research data are well described in Michener et al (1997)[^1], but there are many other kinds of metadata with different purposes.[^2] When publishing datasets, research data repositories typically provide guidance on metadata needs, as seen [here](https://edirepository.org/resources/creating-metadata-for-publication), for example.
...
3. Spread the data cleaning workload around! Data cleaning typically demands a HUGE fraction of the total time devoted to working with data [^3] [^4] [^5], and it can be tedious work. Make sure the team shares this workload equitably.
...
* Data cleaning is a complex topic, and entire books have been written on the subject [^6] [^7], For some general considerations on cleaning data, see EDI's "[Cleaning Data and Quality Control](https://edirepository.org/resources/cleaning-data-and-quality-control)" resource
...
* Both the R and Python data science ecosystems have excellent documentation resources that thoroughly cover data cleaning. For R, consider starting with Hadley Wickham's *R for Data Science* [^8] book chapter on [data tidying](https://r4ds.hadley.nz/data-tidy), and for python, check Wes McKinney's *Python for Data Analysis* [^9] book chapter on [data cleaning and preparation](https://wesmckinney.com/book/data-cleaning).

[^6]: Osborne, Jason W. Best practices in data cleaning: A complete guide to everything you need to do before and after collecting your data. Sage publications, 2012.
[^7]: Van der Loo, Mark, and Edwin De Jonge. Statistical data cleaning with applications in R. John Wiley & Sons, 2018. https://doi.org/10.1002/9781118897126
[^8]: Wickham, Hadley, Mine Çetinkaya-Rundel, and Garrett Grolemund. R for data science. " O'Reilly Media, Inc.", 2023. https://r4ds.hadley.nz/
[^9]: McKinney, Wes. Python for data analysis. " O'Reilly Media, Inc.", 2022. https://wesmckinney.com/book
...
### Harmonizing grassland biomass data - an example
...
4. **There is no perfect data format!** Harmonizing data always involves some tradeoffs.


Below we see some examples of harmonized destination formats that are possible for synthesized data.

:::{.panel-tabset}

## Long (Tidy)

Here our grassland biomass data is in long format, often referred to as "tidy" data. There are three rules for tidy data:

1. Each column is one variable.
2. Each row is one observation.
3. Each cell contains a single value.

**Advantages**: clear meaning of rows and columns, ease in filtering/cleaning  
...
![The harmonized grassland data, in long format.](images/long_data_example.png){width="50%"}
...
**Advantages**: compact size, easier for some statistical analyses (ANOVA, for example)  
**Disadvantages**: may be more difficult to clean or filter, multiple observations per row  
**Possible file formats**: Delimited text (tab delimited shown here), spreadsheets, database tables  

![The harmonized grassland data, restructured into wide format.](images/wide_data_example.png){width="50%"}

## Relational (database style)

Below is a schematic of the related tables that comprise the [ecocomDP](https://ediorg.github.io/ecocomDP/)[^10] harmonized data format for biodiversity data. Eight tables are defined, along with a set of relationships between tables (keys), and constraints on the allowable values in each table. Relational formats like this are "normalized" to reduce data redundancy, and increase data integrity.


**Advantages**: reduced redundancy, greater integrity, community standard  
**Disadvantages**: complex, significant metadata needed to describe  
**File format**: Database stores, can be represented in delimited text (CSV)  

[^10]: O'Brien, Margaret, et al. "ecocomDP: a flexible data design pattern for ecological community survey data." Ecological Informatics 64 (2021): 101374. https://doi.org/10.1016/j.ecoinf.2021.101374


![The ecocomDP schema.](images/ecocomDP_schema.jpg){width="50%"}
...
**Disadvantages**: less familiar/accessible to many scientists, few best practices to follow, costs can be high  
**File format**: Parquet files, distributed/cloud databases

![](images/cloud_native.png){width="50%" center=True}
