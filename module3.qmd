...
The [Python Package Index](https://ropensci.org) (PyPI) is the most widely used venue for publishing Python packages. 
...
The [Comprehensive R Archive Network](https://cran.r-project.org) is the most widely used resource for publishing R packages. Packages are assesed for R version compatibility upon publication.
...
From Currier and Sala 2022[^16]. Note that source datasets are properly cited in the Data Availability Statement, meaning an in-text citation is given and the full bibliographic entry is provided in the article reference list (not shown). The DOIs included here are helpful for quickly finding the data.
...
- Have open meetings when possible to help bring in new team members that are interested and willing to contribute.
